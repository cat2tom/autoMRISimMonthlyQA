...
Input:new_added_file_list-cell array containg all the dicom files in one
directory which is returned by listEPIDDicomFile.
...
for k=1:length(tmp_datetime_cell)
    ...
    if ~isempty(key_tmp) % added to deal with no image files. 
